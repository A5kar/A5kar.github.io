...
|[Law Of The Unconscious Statistician](https://en.wikipedia.org/wiki/Law_of_the_unconscious_statistician) (LOTUS)|$$\displaystyle\mathbb E[g(X)]=\sum_x g(x)p_X(x)$$|
...
Defined as $$\sigma_X^2=\text{var}(X)=\mathbb E[(X-\mathbb E[X])^2]$$, variance measures the spread of a PMF. $$\sigma_X=\sqrt{\sigma_X^2}$$ is referred to as standard deviation.

|Property|Formula|
|-|:-:|
|Non negativity (*zero* only for constants)|$$\text{var}(X)\ge0$$|
|Upper bound|$$a\le X\le b$$, $$\displaystyle\text{var}(X)\le\frac{(b-a)^2}{4}$$ (see [PS5 Q5](https://learning.edx.org/course/course-v1:MITx+6.431x+1T2020/block-v1:MITx+6.431x+1T2020+type@sequential+block@Problem_Set_5/block-v1:MITx+6.431x+1T2020+type@vertical+block@ch8-s7-tab5))|
...
|Binomial<br>$$\text{Bin}(n,p)$$|$$\displaystyle{n\choose k}p^k(1-p)^{n-k}$$|$$np$$|$$np(1-p)$$|
...
According to [independence](/2022/01/05/conditioning-and-independence.html#independence) of two events, $$X$$ is independent from $$A$$ if $$p_{X\lvert A}(x)=p_X(x)\mathbb P(A), \forall x$$ (see [PS4 Q1](https://learning.edx.org/course/course-v1:MITx+6.431x+1T2020/block-v1:MITx+6.431x+1T2020+type@sequential+block@Problem_Set_4/block-v1:MITx+6.431x+1T2020+type@vertical+block@ch6-s7-tab1)). Similarly, $$X$$ is independent from $$Y$$ if $$p_{X,Y}(x,y)=p_X(x)p_Y(y), \forall x,y$$. In such case, below properties hold.
