...
As for the variance of quantile estimators (e.g. median or $$50$$-th percentile), consider $$X_i\overset{\text{i.i.d.}}{\sim}\mathcal P$$, where $$F_X(q)$$ is the CDF of such $$\mathcal P$$ at $$q$$-th quantile and $$Y_i(q)=\mathbb 1(X_i\le q)$$. Accordingly, $$Y_i(q)\overset{\text{i.i.d.}}{\sim}\text{Ber}(p)$$, where $$p=F_X(q)$$, and therefore $$\sqrt n(\bar Y_n(q)-p)\xrightarrow{(d)}\mathcal N(0,p(1-p))$$ (see [CLT](/2022/02/21/introduction-to-statistics.html#clt_mean)).
