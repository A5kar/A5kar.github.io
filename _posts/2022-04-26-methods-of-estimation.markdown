...
&=(1-\alpha)\int_{-\infty}^\theta f(x)dx-\alpha\int_\theta^\infty f(x)dx=\mathbb P(X\le\theta)-\alpha\end{align}$$
...
<iframe width='100%' height='470' src='https://rdrr.io/snippets/embed/?code=n%20%3C-%201e4%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%23%20sample%20size%0Aalpha%3C-4%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%23%20gamma%20shape%0Abeta%3C-2%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%23%20gamma%20rate%0Ax%3C-rgamma(n%2Cshape%3D4%2Crate%3D2)%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%23%20gamma%20distribution%0Ap%3C-.95%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%23%20quantile%20probability%0A%0A%23%20functions%20to%20minimize%0AE_squared%3C-function(theta)%20mean((x-theta)%5E2)%0AE_abs%3C-function(theta)%20mean(abs(x-theta))%0AE_check%3C-function(theta%2C%20p)%0A%20%20mean(sapply(x-theta%2C%20function(u%2Ca%3D.95)%0A%20%20%20%20ifelse(u%3C%3D0%2C(a-1)*u%2Ca*u)))%0A%0A%23%20finding%20the%20minimum%20of%20each%20function%0Aopt_squared%3C-optim(.5%2CE_squared%2Cmethod%3D%22BFGS%22)%0Aopt_abs%3C-optim(.5%2CE_abs%2Cmethod%3D%22BFGS%22)%0Aopt_check%3C-optim(.5%2CE_check%2Cmethod%3D%22BFGS%22)%0A%0A%23%20print%20comparison%20between%20true%2C%20data%20from%20sample%20and%20estimates%0Asprintf(%22mean%3A%20true%20%3D%20%25.2f%20%2F%20sample%20%3D%20%25.2f%20%2F%20estimate%20%3D%20%25.2f%22%2C%0A%20%20%20%20%20%20%20%20alpha%2Fbeta%2Cmean(x)%2Copt_squared%24par)%0Asprintf(%22median%3A%20true%20%3D%20%25.2f%20%2F%20sample%20%3D%20%25.2f%20%2F%20estimate%20%3D%20%25.2f%22%2C%0A%20%20%20%20%20%20%20%20qgamma(.5%2Cshape%3Dalpha%2Crate%3Dbeta)%2Cquantile(x%2C.5)%2Copt_abs%24par)%0Asprintf(%22p-quantile%3A%20true%20%3D%20%25.2f%20%2F%20sample%20%3D%20%25.2f%20%2F%20estimate%20%3D%20%25.2f%22%2C%0A%20%20%20%20%20%20%20%20qgamma(p%2Cshape%3Dalpha%2Crate%3Dbeta)%2Cquantile(x%2Cp)%2Copt_check%24par)' frameborder='0'></iframe>

As for the variance of the quantile estimator, we can exploit [CLT](/2022/02/21/introduction-to-statistics.html#clt_mean) and the convergence of the [empirical CDF](/2022/07/15/hypothesis-testing.html#ecdf), $$F_n(q)=\frac1n\sum_{i=1}^n\mathbb 1(X_i\le q)\xrightarrow{\mathbb P}F_X(q)=p$$, that is $$\sqrt n(F_n(q)-p)\xrightarrow{(d)}\mathcal N(0,p(1-p))$$. Since $$q=F_X^{-1}(p)$$ and $$\frac{\partial p}{\partial q}=f_X(q)$$, it follows that $$\frac{\partial q}{\partial p}=\frac{\partial}{\partial p}F_X^{-1}(p)=\frac1{f_X(q)}$$. Therefore, thanks to [Delta method](/2022/03/24/foundation-of-inference.html#delta) we obtain that $$X_{(np)}$$, that is $$np$$-th order statistic, used as estimator of $$q$$ ($$\alpha$$ quantile of $$X$$), is distributed as follows.

$$\begin{align}
\sqrt n(F_X^{-1}(F_n(q))-F_X^{-1}(p))&\xrightarrow{(d)}\frac1{f_X(q)}\mathcal N(0,p(1-p))\\
...
$$\begin{align}
\sqrt n(\hat\Theta_n-\theta)\xrightarrow{(d)}\mathcal N(0,J^{-1}(\theta)K(\theta)J^{-1}(\theta))\text{, where }J(\theta)=\mathbb E[\mathbf H_\theta\rho(X,\theta)]
\end{align}$$

If $$\rho(X,\theta)=-\ell(X,\theta)$$, then the estimator is MLE and $$J(\theta)=K(\theta)=I(\theta)$${% if jekyll.environment == "development" %} (see [Stats L12 Q4](https://learning.edx.org/course/course-v1:MITx+18.6501x+2T2021/block-v1:MITx+18.6501x+2T2021+type@sequential+block@u03s05_methodestimation/block-v1:MITx+18.6501x+2T2021+type@vertical+block@u03s05_methodestimation-tab4)){% endif %}.
...
- few more technical conditions (see [MLE](/2022/04/26/methods-of-estimation.html#mle)).
...
Let us revisit estimation variance $$\Gamma_\delta(\mu)$$ for the location of Laplace and Cauchy distributions, including for $$\delta\rightarrow0$$ and $$\delta\rightarrow\infty$$, associated with the median and mean estimators{% if jekyll.environment == "development" %} (see [Stats L12 Q9](https://learning.edx.org/course/course-v1:MITx+18.6501x+2T2021/block-v1:MITx+18.6501x+2T2021+type@sequential+block@u03s05_methodestimation/block-v1:MITx+18.6501x+2T2021+type@vertical+block@u03s05_methodestimation-tab9)){% endif %}.
...
|$$\displaystyle\frac\partial{\partial x}f^{-1}(x)$$<br>where $$x=f(y)$$|$$\begin{align}\displaystyle1=\frac{\partial x}{\partial x}&=\frac{\partial f(y)}{\partial x}=\frac{\partial f(y)}{\partial y}\frac{\partial y}{\partial x}=f'(y)\frac{\partial y}{\partial x}\\\implies\frac{\partial y}{\partial x}&=\frac\partial{\partial x}f^{-1}(x)=\frac1{f'(f^{-1}(x))}\end{align}$$|
