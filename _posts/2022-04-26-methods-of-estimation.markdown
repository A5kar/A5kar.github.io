...
|Categorical with parameter $$\mathbf p$$|$$\prod_{j=1}^d p_j^{a_j}$$<br>$$a_j=\sum_{i=1}^n\mathbb 1(X_i=j)$$|$$\hat{\mathbf p}=\begin{bmatrix}\frac {a_1}n\\\vdots\\\frac{a_d}n\end{bmatrix}$${% if jekyll.environment == "development" %}<br>(see [Stats L10 Q3](https://learning.edx.org/course/course-v1:MITx+18.6501x+2T2021/block-v1:MITx+18.6501x+2T2021+type@sequential+block@u03s03_methodestimation/block-v1:MITx+18.6501x+2T2021+type@vertical+block@u03s03_methodestimation-tab3)){% endif %}|
