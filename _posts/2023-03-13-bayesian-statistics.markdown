...
Note that computations of probability of error $$\mathbb P(\text{error})=\mathbb P(\Theta\neq\hat\Theta_n)$$, and mean square error $$\text{MSE}(\hat\Theta_n)=\mathbb E[(\hat\Theta_n-\Theta)^2]$$ benefit from the [total probability / total expectation theorem](/2022/01/05/conditioning-and-independence.html#main_three_tools).
